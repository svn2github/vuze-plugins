...
import com.aelitis.azureus.plugins.xmlhttp.exceptions.*;
...
public class XMLRequestProcessor2 {
    ...
    protected void process(String client_ip, RPRequestAccessController access_controller, XMLElement response_xml, PluginInterface pi) throws Exception {
        ...
        if (req_obj.getMethod() == null) {
        	throw new RPMalformedXMLException("no method specified in request");
        }
    ...
    public XMLElement describeError(RPException rpe) {
        ...
        if (rpe instanceof RPExtendedException) {
        	xe.addContent(((RPExtendedException)rpe).getSerialisationMessage());
        }
        else {
        	xe.addContent(RPUtils.exceptionToString(rpe));
        }
    ...
    protected Object deserialiseObject(SimpleXMLParserDocumentNode node, Class cla) throws Exception {
        ...
        try{
            ...
            Field[] fields = cla.getFields(); // getDeclaredFields() doesn't do subclasses.
            for (int i=0;i<fields.length;i++) {
                ...
                if (type.isArray()) {
                    ...
                    if ( entries.length > bits.length ){
                       	...
                       	String str = "Mismatch between arg count in signature (" + bits.length + ") and args supplied (" + entries.length + ")";

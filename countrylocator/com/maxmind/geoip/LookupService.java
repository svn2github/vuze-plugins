 ...
 * ==========================
 * - Added TL, CS and ZZ Country Codes, with reserved padding in case the official
 *   GeoIP.dat ever expands its country list
 * - Hashmaps size/filling based on array size
 * - Return UNKNOWN_COUNTRY for out of bound array
 * - Added getCountryCodes
 * - Made UNKNOWN_COUNTRY public so we can do easy comparisons
...
public class LookupService {
	...
	"N/A","N/A","N/A","N/A","N/A","Timor-Leste",
	"Czechoslovakia","IETF RESERVED (Spoofed IP)"};

    private final static HashMap hashmapcountryCodetoindex = new HashMap(countryCode.length);
    private final static HashMap hashmapcountryNametoindex = new HashMap(countryCode.length);
    ...
    private synchronized int seekCountry(long ipAddress) {
    	//System.out.print(GeoIP_Tree.hex32Str(ipAddress) + " = ");
	...
	int [] x = new int[2];
		...
		try {
            ...
            //if (((depth + 1) & 7) == 0) System.out.print(" ");

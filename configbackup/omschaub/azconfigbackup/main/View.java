...
public class View implements UISWTViewEventListener {
    ...
    static final String VIEWID = "AZConfigBackup_View";
    ...
    public static Composite composite_for_tab1;
    static Composite composite_for_tab4;
  ...
  View(PluginInterface pluginInterface) {
    ...
    if (pluginInterface.getPluginconfig().getPluginBooleanParameter("AutoBackupConfig")) {
    	Tab4Utils.backupButtonRun(1);
    }
  }
  ...
  private void initialize(Composite parent) {
		...
		if (pluginInterface.getPluginconfig().getPluginIntParameter("Azconfigbackup_sash_info", 60) == 0)
            pluginInterface.getPluginconfig().setPluginParameter("Azconfigbackup_sash_info", 87);
		...
		//composite for tab4
		composite_for_tab4 = new Composite(tab,SWT.NULL);
	    ...
	    tab4_comp_layout.marginHeight = 0;
	    tab4_comp_layout.marginWidth = 0;
    ...
    if (pluginInterface.getPluginconfig().getPluginIntParameter("Azconfigbackup_sash_info", 60) > 100)
        pluginInterface.getPluginconfig().setPluginParameter("Azconfigbackup_sash_info", 100);
     sash.setWeights(new int[] {pluginInterface.getPluginconfig().getPluginIntParameter("Azconfigbackup_sash_info", 60),100 - pluginInterface.getPluginconfig().getPluginIntParameter("Azconfigbackup_sash_info", 60)});
     
     tab.addListener(SWT.Resize, new Listener() {
        public void handleEvent(Event e) {
          int[] weights = sash.getWeights();
          int iSashValue = weights[0] * 100 / (weights[0] + weights[1]);
          pluginInterface.getPluginconfig().setPluginParameter("Azconfigbackup_sash_info", iSashValue);
        }
      });

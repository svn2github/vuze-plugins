...
public class UI implements MdiEntryCreationListener
{
	...
	public static final String VIEW_ID = "AirVPN_View";
	...
	public UI(PluginInterface pi, UISWTInstance swtInstance) {
		...
		menuItemShowView = uiManager.getMenuManager().addMenuItem(
				...
				"ConfigView.section." + PluginAir.CONFIG_SECTION_ID);
	...
	public MdiEntry createMDiEntry(String id) {
		...
		MdiEntry entry = mdi.createEntryFromSkinRef(null, VIEW_ID, "airvpnview",
				"AirVPN", null, null, true, null);
		entry.setTitleID("ConfigView.section." + PluginAir.CONFIG_SECTION_ID);
		
		final ViewTitleInfo viewTitleInfo = new ViewTitleInfo() {
			public Object getTitleInfoProperty(int propertyID) {
				if (propertyID == ViewTitleInfo.TITLE_INDICATOR_TEXT) {
					int statusID = PluginAir.instance.checker.getCurrentStatusID();
					
					LocaleUtilities texts = UI.this.pi.getUtilities().getLocaleUtilities();

					if (statusID == Checker.STATUS_ID_OK) {
						return texts.getLocalisedMessageText("airvpn.indicator.ok");
					}
					if (statusID == Checker.STATUS_ID_BAD) {
						return texts.getLocalisedMessageText("airvpn.indicator.bad");
					}
					if (statusID == Checker.STATUS_ID_WARN) {
						return texts.getLocalisedMessageText("airvpn.indicator.warn");
					}
					return null;
				}
		...
		PluginAir.instance.checker.addListener(checkerListener);

		entry.addListener(new MdiCloseListener() {
			public void mdiEntryClosed(MdiEntry entry, boolean userClosed) {
				if (PluginAir.instance.checker != null) {
					PluginAir.instance.checker.removeListener(checkerListener);
				}
			}
		});
		return entry;
	...
	private void addSkinPaths() {
		String path = "com/vuze/plugin/azVPN_Air/skins/";

		String sFile = path + "skin3_" + PluginAir.CONFIG_SECTION_ID;

		ClassLoader loader = PluginAir.class.getClassLoader();
